...
public class MaxOps {
	...
	//---------------------------------------------------
	  ...
	  void writeMatrix(float[][] a, String fileName) {
	    p5.PrintWriter out = p5.createWriter(fileName+".csv");
	    for (int i=0; i<a.length; i++) {
	      for (int j=0; j<a[i].length; j++) {
	        if (j==a[i].length-1)out.print(p5.str(a[i][j]));
	        else out.print(p5.str(a[i][j])+",");
	      }
	      out.println();
	    }
	    out.flush();
	  ...
	  PImage pictureMatrix(float[][] a) {
	    PImage pic = createImage(getM(a), getN(a), p5.RGB);
	    pic.loadPixels();
	    for (int i=0; i<a.length; i++) {
	      for (int j=0; j<a[i].length; j++) {
	        pic.pixels[j+(i*getM(a))] = p5.color(a[i][j]);
	      }
	    }
	    pic.updatePixels();
	    return pic;
	  ...
	  float[][] createPackMatrix(PImage img, int c) {
	    if (c>=0 && c<=5) {
	      ...
	      for (int i=0; i<a.length; i++) {
	        for (int j=0; j<a[i].length; j++) {
	          if (c==0)a[i][j] = p5.red(img.get(j, i));
